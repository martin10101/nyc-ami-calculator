...
def _selected_for_ami_mask(d: pd.DataFrame) -> pd.Series:
    ...
    The selection semantics follow the project spec:
      * If an explicit affordability flag is present (e.g. a column in FLEX_HEADERS["AFF"]),
        then any truthy/ticked value marks the unit as selected.
      * If a dedicated signed AMI column is present, any non-empty value marks selection.
      * Otherwise, the AMI column itself is treated as a selection indicator.  ANY non‑blank
        value (numbers like 0.6, 0.7, "Yes", "✓", etc.) in the AMI column means that the
        unit is part of the affordable set.  The numeric value is ignored for selection
        purposes—actual AMI bands will be reassigned later.
      * As a final fallback, the legacy behaviour keeps compatibility by looking at AMI_RAW
        near 0.6 or ticked values.

    Only units with non‑missing NET SF are eligible.
    """
...
def _adjust_wavg_down(assigned: np.ndarray, sf: np.ndarray, avg_high: float) -> np.ndarray:
    ...
    the overall SF-weighted average does not exceed avg_high.  This helper
    operates greedily: it looks at units assigned to the highest AMI bands and
    steps them down one band at a time.

    Parameters
    ----------
    assigned : np.ndarray
        Current band assignments (floats such as 0.40, 0.60, ...).
    sf : np.ndarray
        Square footage weights for each unit.
    avg_high : float
        Maximum allowed weighted average.

    Returns
    -------
    np.ndarray
        Possibly modified assignments meeting the weighted average threshold.
    """
...
def _try_exact_optimize(sf: np.ndarray, floors: np.ndarray, beds: np.ndarray, low_share=0.20, high_share=0.21,
    ...
    def build_lp(maximize_wavg=True, wavg_floor=None):
        ...
        if maximize_wavg:
            ...
            # Secondary objective: penalise high floors for 40% AMI units by minimising
            # the total floor numbers assigned to the 40% band.  Since PuLP always
            # maximises, multiply by -1 so higher floor assignments reduce the objective.
...
def _heuristic_assign(sf: np.ndarray, mask40: np.ndarray, target_avg=0.60, max_bands=3, floors: Optional[np.ndarray] = None) -> np.ndarray:
    """
    Heuristic assignment when the exact MILP is unavailable.  Starts with all selected
    units at 60% AMI, sets the chosen 40% mask, then greedily raises some units to
    higher bands until the weighted average approaches the target.  When floor
    information is provided the algorithm orders the units so that higher AMI
    bands are preferentially placed on higher floors.
    """
    ...
    # Prefer assigning higher bands to higher floors if floor data available; otherwise shuffle for variety
...
def generate_scenarios(aff: pd.DataFrame, low_share=0.20, high_share=0.21, avg_low=0.59, avg_high=0.60,
    ...
    def build(label, need_family, protect_top):
        ...
        # Assign bands
        # Initialise assigned to None so it exists regardless of try_exact flag
        assigned = None
        if try_exact:
            # Exact MILP assignment may return None if solver unavailable or infeasible
        ...
        # Fallback to heuristic when exact optimiser is disabled or fails
        if assigned is None:
    ...
    # Build up to three scenarios (S1/S2/S3).  If a scenario fails to meet the
    # compliance targets (raises RuntimeError), skip it gracefully.  This allows
    # downstream logic to continue with the remaining feasible scenarios rather
    # than aborting the entire allocation.
    scenarios = []
    for label, need_family, protect_top in [
        ("S1", False, 0),
        ("S2", True,  0),
        ("S3", False, max(exempt_top_k_floors, 0)),
    ]:
        try:
            scenarios.append((label, build(label, need_family=need_family, protect_top=protect_top)))
        except Exception:
            # Skip scenarios that cannot satisfy the constraints
            continue
    if not scenarios:
        raise RuntimeError("No feasible scenarios under current constraints.")
    out_raw = {label: blob for label, blob in scenarios}
...
def prune_to_top_k(scens: Dict, top_k: int):
    # Higher weighted average AMI is better, but a lower average floor for 40% units
    # (i.e. smaller vscore) is preferred.  Therefore subtract vscore to penalise
    # scenarios that place 40% units on higher floors.  Also penalise scenarios
    # exceeding the band count limit.
    def score(blob):
        m = blob["metrics"]
...
def allocate_with_scenarios(
    ...
    aff = d.loc[sel].copy()
    # Attempt to build scenarios.  If MILP (exact) mode is requested and fails due to
    # infeasibility or over‑constraints, fall back to the heuristic by retrying with
    # try_exact=False.  This ensures that users still receive a best‑effort allocation
    # rather than a hard failure when the exact solver cannot satisfy all rules.
    try:
        ...
        if use_milp:
            ...
            # re‑raise if already using heuristic
            raise
