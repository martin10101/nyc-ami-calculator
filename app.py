...
from ami_core import allocate_with_scenarios  # <- the upgraded core in this bundle
...
def load_any_table(file_bytes: bytes, filename: str, sheet: Optional[str] = None) -> pd.DataFrame:
    ...
    if name.endswith(".docx"):
        ...
        for t in doc.tables:
            ...
            if any("sf" in str(c).lower().replace(" ","") for c in df.columns): best = df; break
            best = best or df
...
async def preview(
    ...
    # Build a compact JSON preview grouped by band for each scenario
    ...
    def buckets(label):
        ...
        for _,r in aff_only.iterrows():
            v = float(r[col])
            k = {0.4:"40",0.6:"60",0.7:"70",0.8:"80",0.9:"90",1.0:"100"}[round(v,1)]
            out[k].append({
                "APT": r.get("APT"),
    ...
    with pd.ExcelWriter(buf, engine="XlsxWriter") as w:
        ...
        for lab in [c.replace("Assigned_AMI_","") for c in full.columns if c.startswith("Assigned_AMI_")]:
            aff_br[[*aff_br.columns]].assign(**{f"Assigned_AMI": full.loc[full[f"Assigned_AMI_{lab}"].notna(), f"Assigned_AMI_{lab}"].to_numpy()}) \
                 .to_excel(w, index=False, sheet_name=f"Breakdown_{lab}")
        pd.DataFrame([
