...
def test_beach_115_golden_truth():
    ...
    assert abs(scenario_s1['waami'] - 0.6) < 1e-6
    assert set(scenario_s1['bands']) == {40, 60, 80}

    assignments = {str(u['unit_id']): int(u['assigned_ami'] * 100) for u in scenario_s1['assignments']}

    expected_assignments = {
        '201': 40, '202': 40, '203': 40, '204': 40, '205': 40, '206': 40, '207': 40, '208': 40,
        '209': 60, '210': 60, '211': 60, '212': 60,
        '301': 60, '302': 60, '303': 60, '304': 60, '305': 60, '306': 60, '307': 60, '308': 60, '309': 60, '310': 60, '311': 60, '312': 60,
        '401': 60, '402': 60, '403': 60, '404': 60,
